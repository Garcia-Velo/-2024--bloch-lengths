...
def basis_definitions(dim: list[int]) -> tuple[np.ndarray, dict[tuple[int, ...], np.ndarray]]:
    ...
    local_bases = [generate_pauli_basis() for _ in dim]
...
def main_optimization(run_kwargs: dict[str, Any], grid: dict[str, Any], data_dir: Path) -> None:
    ...
    mask_ent, mask_sep, mask_mix = grid["mask_ent"], grid["mask_sep"], grid["mask_mix"]
    ...
    for counter, (idx, jdx) in enumerate(zip(indices[0], indices[1]), 1):
        ...
        if mask_ent[idx, jdx]:
            ...
            max_result = run_repeat(run_kwargs)
...
def plot_results(ent_max: np.ndarray, ent_min: np.ndarray, grid: dict[str, Any], plots_dir: Path) -> None:
    ...
    mesh0 = ax[0].pcolormesh(T, Z, ent_max, shading='auto', cmap='viridis', vmin=0, vmax=1)
    ax[1].pcolormesh(T, Z, ent_min, shading='auto', cmap='viridis', vmin=0, vmax=1)

    # Plot the width of the entanglement-bound interval.
    DN = np.maximum(0, ent_max - ent_min)
    mesh2 = ax[2].pcolormesh(T, Z, DN, shading='auto', cmap='viridis', vmin=0, vmax=0.42)
    ...
    labels = ['(a)', '(b)', '(c)']
    for i in range(3):
        ax[i].set(xlabel = r"$\sqrt{2} || r_1 || = \sqrt{2} || r_2 ||$", ylabel = r"$||r_{1, 2}||$", xlim = (0, 1.5), ylim = (0, 2),
        ...
        ax[i].plot(t, z_lower, 'grey', linewidth=2, label='Lower bound')
        ax[i].plot(t, z_sep, 'blue', linewidth=2, label='Separable')
        ax[i].grid(True)
    ...
    plt.savefig(plots_dir / "sym_ent.png")
...
if __name__ == "__main__":
    ...
    data_dir = PROJECT_ROOT / "data" / "scripts" / "two_qubits"
    plots_dir = PROJECT_ROOT / "plots" / "scripts" / "two_qubits"
    ...
    neg_max = 2 * np.asarray(compute_negativity(trace_norm_pt=ent["max"]))
    neg_min = 2 * np.asarray(compute_negativity(trace_norm_pt=ent["min"]))
